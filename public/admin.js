...
adminHeroForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const result = await readAdminJson(response, "Hero upload");
    adminHeroStatus.textContent = result.ok ? "Hero image updated." : (result.error || "Hero upload failed.");
    adminHeroFile.value = "";
    adminHeroNote.value = "";
    if (result.ok) {
...
adminAboutImageForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const result = await readAdminJson(response, "About image upload");
